...
public class CourseListResourceTest extends AbstractShiroTest {
	
	private CourseListResource courseListResource;
	...
	public void initialize() {
		courseListResource = new CourseListResource();
		courseListResource.setRepository(courseInfoRepository);
		...
		courseListResource.setRequest(request);
	}
	...
	public void testCourseList() throws IOException {
		...
		RequestAttributes.setRequestPager(courseListResource.getRequest(), new Pager());
		
		Representation result = courseListResource.getResource();
		...
		Assert.assertTrue(data.get("courses").isJsonArray());
	}
